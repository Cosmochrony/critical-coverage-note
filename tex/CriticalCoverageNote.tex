...
\begin{document}
  ...
  \begin{abstract}
    ...
    $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ --- the central asymptotic variable of the O25--O28 window
    calibration --- converges to the constant $n_* = 22$, and the critical coverage
    ...
    A structural corollary is that the auto-calibrated fitting window closes deterministically for
    $q > 99{,}689$.
    ...
    of the substrate, exploration creates no further spectral novelty per vertex --- so shellwise
    novelty is a local combinatorial phenomenon, and no continuum-scaling channel opens from this
    observable.
  \end{abstract}
  ...
  \section{The saturation-depth problem}
    ...
    Let $q$ be an odd prime, $G_q = \Heis_3(\mathbb{Z}/q\mathbb{Z})$ with multiplication
    $(a,b,\gamma)(a',b',\gamma') = (a+a',\,b+b',\,\gamma+\gamma'+ab')$, and $\Cay(G_q, S_q)$ its
    Cayley graph with symmetric generating set $S_q = \{X^{\pm 1}, Y^{\pm 1}\}$, $X = (1,0,0)$,
    $Y = (0,1,0)$.
    ...
    where $u$ is the uniform seed and $\rho_c$ is the Weil representation
    ...
    $N(q) = \lfloor 1.3\sqrt{q}\rfloor + 2$ is the processing cap of the production measurement.
    ...
    central open direction of the window calibration, and O28 established model-free that
    $\none(q)/q \to 0$.
    ...
    \begin{proposition}[Volume growth, $D=4$]
      \label{prop:growth}
      On $\Cay(G_q, S_q)$ the homogeneous dimension is $D = 4$
      (Bass--Guivarc'h~\cite{Bass1972,Guivarch1973}; O9~\cite{Beau2026a13}), so for $n \ll q$,
      \begin{equation}
    ...
    \begin{proposition}[Depth--coverage identity]
      ...
      \begin{equation}
        ...
        \label{eq:reduction}
      \end{equation}
      The law of $\none(q)$ and the law of $\xone(q)$ are therefore the same statement in two
      gauges; the results below establish $\none \to 22$ and $\xone \asymp q^{-2}$ jointly.
    \end{proposition}
  ...
  \section{The exact interval theorem}
    ...
    \begin{proof}
      The map $\beta:(a,b,\gamma)\mapsto b$ is a homomorphism to $(\Fq,+)$ with
      $\beta(S_q) = \{\pi_q(-1), 0, \pi_q(1)\}$, so $\beta(B_n) \subseteq I_n$; conversely
      $Y^k \in B_n$ for $\lvert k\rvert \le n$.
    \end{proof}
  ...
  \section{Main theorem}
    ...
    \begin{theorem}[Constant saturation depth and inverse-square coverage]
      \label{thm:main}
      Let $\epssat = 10^{-3}$, $M = 5$, and let $\none(q)$ be the statistic~\eqref{eq:n1def} with
      cap $N(q) = \lfloor 1.3\sqrt{q}\rfloor + 2$.
      \begin{enumerate}[nosep]
        \item \textbf{Deterministic part.}
          For every prime $q \ge 311$ and arbitrary blocks with $C_i \ne 0$:
          \begin{equation}
            \none(q) \le 22,
            \qquad
    ...
    \begin{proof}
      ...
      The cap satisfies $2\lfloor N(q)^2/4\rfloor \le 0.845\,q + 2.6\sqrt{q} + 2 < q$ for
      $q \ge 311$, so Lemma~\ref{lem:isometry} identifies $s_n(q) = s_n$ on the whole computed
      range, and $s_n > 18{,}000$ for every $n \ge 23$ by Lemma~\ref{lem:cubic}; hence
    ...
    \begin{corollary}[The auto-calibrated window closes at large $q$]
      \label{cor:window}
      The lower window edge of O12~\cite{Beau2026a16} is $n_0(q) = \min\{n : \Bball \ge q\}$.
      For $q > \lvert B_{22}\rvert = 99{,}689$ (and $q \ge 311$ as above), $n_0(q) \ge 23 > 22 \ge
      \none(q)$: the auto-calibrated fitting window $[n_0, \none]$ is deterministically empty.
  ...
  \section{The transitional regime and the measured coverage}
    ...
    reproducing the O28 production values exactly on $q \le 211$) and the derived coverage
    ...
    \begin{remark}[Falsifiable finite-$q$ prediction]
      \label{rem:prediction}
      At $q \in \{809, 1009, 1201\}$ the expected number of active resonance conditions per block
      near depth $22$ is still of order one, so the theorem predicts $\none(q)$ \emph{strictly
      below} $22$ there, with stabilisation at $\none = 22$ only for $q \sim 10^4$--$10^5$.
      This is a sharp, pre-stated discriminating test for any future measurement campaign at those
      primes.
    \end{remark}
  ...
  \section{Degeneracy of the coverage-collapse ansatz}
    ...
    \begin{proposition}[The collapse is degenerate]
      \label{prop:collapse}
      By Theorem~\ref{thm:interval}, $r_c(n) \le 9(2n+3)$, so at fixed coverage
      $x = \Bball/q^2$ (i.e.\ $n \asymp \sqrt{q}$) the rank is $O(\sqrt{q}) = o(q)$.
      The only function compatible with~\eqref{eq:star} is therefore $\Phi \equiv 0$, which
      controls no threshold front: the natural support scale of the rank is $n/q$, not
      $\Bball/q^2$.
    \end{proposition}
    ...
    sphere counts), not diffusive: as noted in Proposition~\ref{prop:gap}, the spectral gap
  ...
  \section{Status and consequences}
    ...
    \paragraph{Consequence for the spatial-limit programme.}
      ...
      This removes the only identified conditional mechanism supporting a harmonic confinement in
      that limit; the existence of a spatial limit operator (hypothesis [H-L] of the
      emergent-geometry sub-programme) is unaffected in either direction by the present result,
      which is spectral only.
    ...
    \subsection{Interpretive outlook}
      ...
      exploration creates essentially no new spectral directions per visited vertex.
      ...
      their extraction window closes once the substrate outgrows the horizon ball
